...
export default function ContasPagarPage() {
  ...
  const carregar = useCallback(async () => {
    const res = await api.get<ContaPagar[]>("/contas_pagar");
    setRows(res.data);
  }, []);
  ...
  const salvar = async () => {
    ...
    try {
      if (editId) {
        await api.put(`/contas_pagar/${editId}`, payload);
        toast.success("Conta atualizada!");
      } else {
        await api.post("/contas_pagar", payload);
  ...
  const confirmarExclusao = async () => {
    if (!excluirId) return;
    try {
      await api.delete(`/contas_pagar/${excluirId}`);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            {rows.map((c) => (
              <TableRow key={c.ID_CONTA_PAGAR}>
                <TableCell>{c.ID_CONTA_PAGAR}</TableCell>
                ...
                <TableCell className="text-right">
                  ...
                  <Button
                    ...
                    onClick={() => setExcluirId(c.ID_CONTA_PAGAR)}
                  >
